...
function initializeWebViewEvents() {
    ...
    // This API is avaiable only when accessing the Blazor app from a webiew. If for any reason this is not available, as alternative you can use direct JS functions instead of messages, see https://learn.microsoft.com/en-us/microsoft-edge/webview2/get-started/wpf#step-9---scripting
    if (window.chrome.webview) {
        window.chrome.webview.addEventListener('message', receiveMessageFromWebView);
    }
}

// Initializes the helper reference needed to interop with dotnet.
